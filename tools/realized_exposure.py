...
def main(argv=None) -> int:
    ...
    report: dict = {
        ...
        "method": "Episodes ended per cohort per iteration from the survival telemetry, binned by that cohort's intensity from the curriculum trace, summed over the run.",
    ...
    print(f"{'run':{width}} {'episodes':>10} " + " ".join(f"{l:>16}" for l in LABELS) + f" {'<= envelope':>12}")
    for name, run in report["runs"].items():
        cells = " ".join(f"{100 * run['share_by_difficulty'][l]:15.1f}%" for l in LABELS)
        print(f"{name:{width}} {run['total_episodes']:>10,} {cells} {100 * run['share_at_or_below_the_envelope']:11.1f}%")
    print("\nshare of all training episodes whose cohort sat in that intensity band")
