...
ALL_CELLS = ("phys_100", "phys_125", "phys_150", "phys_175", "phys_200", "ch_fric_150", "ch_fric_200",
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"{'arm':11s} {'final λ':>8s} {'exp':>4s} {'first':>6s} {'dec':>4s} | {'p175':>6s} {'p200':>6s} {'PRIM':>6s} | {'m300':>6s} {'c300':>6s} {'j300':>6s} {'ASYM':>6s} | {'push2':>6s} {'push3':>6s} | {'p150':>6s}")
    for mode in order:
        e = arms.get(mode)
        if not e:
            continue
        ...
        print(f"{mode:11s} {lam_s} {m.get('expansions', '-'):>4} {str(m.get('first_fire_iteration', '-')):>6s} {str((m.get('scalar_decreases_in_trace') or 0) + (m.get('vector_decreases_in_trace') or 0)):>4s} | {fmt(c.get('phys_175'))} {fmt(c.get('phys_200'))} {fmt(e['primary_phys175_200'])} | {fmt(c.get('ch_mass_300'))} {fmt(c.get('ch_com_300'))} {fmt(c.get('ch_joint_300'))} {fmt(e['asym_primary_mass_com_joint_3x'])} | {fmt(c.get('ch_push_200'))} {fmt(c.get('ch_push_300'))} | {fmt(c.get('phys_150'))}")
        if vec:
