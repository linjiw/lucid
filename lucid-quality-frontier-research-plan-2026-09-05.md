...
September 5 late-evening outcome: the [fixed-initial diagnostic](lucid-initial-hold-results-2026-09-05.md) completed and breaches origin-relative tracking retention without expansion. The next executable study is therefore the fresh-versus-restored optimizer-history control. Commit `e981e97` implements parameter-binding reconstruction, a before-rollout runtime seam, and paired smoke/pilot plans. The longer run is conditional on complete smoke receipts, exact fresh-arm checkpoint parity, and verified analysis. This is a candidate-mechanism test; an expansion-only quality veto remains deferred.
...
The [optimizer-history preparation memo](lucid-optimizer-history-control-2026-09-05.md) records an implemented strict transplant and exact real-checkpoint numerical validation, plus the outstanding named-parameter binding and simulator smoke requirements. Its synthetic CPU experiment is not a training-performance result.
