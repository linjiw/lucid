...
| `docs/` | TACE design, LUCID-MC/IROS plan, cross-domain curriculum-learning review |
| `lucid-original-paper.md`, `lucid-proposal.md`, `lucid-sonic.md` | source documents |
| `receipts/manifests/` | **every experiment receipt** (JSON, git SHA + seeds + verified/not-verified split), mirrored from `/data/robotixx/lucid-sonic/manifests/` |
| `env/lucid_env.sh` | the environment script every run must source (conda env, TMPDIR, PYTHONPATH, threads) |
...
2. Environment: conda env `sonic` = IsaacLab 2.3.2 / IsaacSim 5.1.0 / torch 2.7.0+cu128 (sm_120).
   Adapt `env/lucid_env.sh` paths (`LUCID_ROOT`, `LUCID_REPO`, conda path) and `source` it before any python.
...
4. CPU tests (no GPU): `pytest tests/practice_utility/` inside the submodule (1,153 tests).
