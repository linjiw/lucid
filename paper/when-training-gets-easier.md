...
## 9. Proposed: The Range-Expansion Curriculum

Sections 4–7 fix what a curriculum may do and what it may listen to. The curriculum we
propose follows in four sentences.

- Test proposed conditions before expanding a training range.
- Allow different physical parameters to expand at different times.
- Check whether combined changes cause failures.
- Continue practicing earlier conditions while keeping the admitted ranges from shrinking.

Concretely, the 1,024 environments are split into a retained set that rehearses earlier
ranges (25%), the active range (62.5%), and a probe set placed one step beyond the active
range on one channel (12.5%). Survival on the probe set decides whether that channel
expands. A periodic probe at the joint corner of all proposed ranges can hold an expansion
that passes channel by channel but fails in combination. No decision can shrink a range.

**What is measured so far.** A scalar version of the probe gate, trained from scratch,
expanded λ from 1.0 to 1.5 in four evidence-triggered steps at iterations 3,362, 4,103,
4,790, and 5,891, recorded probe telemetry on 7,990 of 8,000 iterations, and applied no
decrease. A stale global return guard froze expansion 914 times after the ceiling was
reached; the final design uses probe-relative evidence instead. The per-channel and
joint-corner components are implemented as a prototype and not yet evaluated.

**The test it must pass.** Feedback earns credit only against a preset expansion schedule
that reaches the same final ranges with the same compute and the same training mixture. If
the adaptive curriculum ties the schedule, range width and monotone exposure explain the
result and adaptive timing does not. The comparison is paired by seed, five seeds, with the
held-out endpoints and margins of Section 5 frozen in advance. A tie or loss narrows the
paper to Sections 4–7; it does not reopen the shrinkage pathway those sections close.
...
to a noninferiority statement. The MuJoCo replay uses our own randomization and the ONNX
