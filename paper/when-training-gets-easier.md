...
| Channel | Configured range and units | Timing |
|---|---|---|
...
| Recorded mode | Label | Recorded range assignment |
|---|---|---|
| lucid_rg | A; shrink permitted | All 1,024 environments at λ |
| lucid_s4_rg | B; shrink permitted | Four fixed cohorts of 256 at λ/4, λ/2, 3λ/4, λ |
| lucid_ratchet_rg | N; never-shrink | All environments at λ; decreasing proposals rejected |
| fixed | F; fixed randomization | All environments at λ=1 |
...
For protected condition c and metric m in {global,local}, compare each checkpoint with its own campaign's frozen-origin panel. Define $D(t)=\max_{c,m}100(E_m(t,c)/E_m(0,c)-1)$ and $L(t)=\max_c100(S(0,c)-S(t,c))$, where S is completion fraction. A sampled checkpoint passes exactly when D(t)≤10% and L(t)≤2 percentage points. We evaluate iterations 250, 500, 1,000, 1,500, and 2,000. This empirical development rule is not a simultaneous confidence guarantee between checkpoints or across trained origins.
...
| Arm / continuation seed | Nominal error; C | Envelope error; C | D; L at 2,000 | Hard Q | All sampled gates |
|---|---|---|---|---|---|
